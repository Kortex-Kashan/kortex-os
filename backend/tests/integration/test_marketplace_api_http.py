"""M7 real-HTTP regression coverage for the Marketplace catalog.
...
M7 requires:
    ...
    valid session token, "marketplace:read"     -> 200, payload.result == []

See `test_workflow_api_http.py`'s module docstring for why `httpx.ASGITransport`
is used here rather than `TestClient` (both drive the app on the *same*
...
_TEST_ROLE = "marketplace-http-test-role"
_CREDENTIAL = "marketplace-http-test-credential"


@pytest_asyncio.fixture
async def kernel(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> AsyncIterator[Kernel]:
    monkeypatch.setenv("KORTEX_STORAGE_DIR", str(tmp_path / "marketplace_http_storage"))
...
def _tenant() -> str:
    return f"tenant-marketplace-http-{uuid.uuid4().hex[:8]}"
...
async def _invoke_listing_list(client: httpx.AsyncClient, token: str | None) -> httpx.Response:
...
async def test_no_authorization_header_returns_401(client: httpx.AsyncClient) -> None:
    response = await _invoke_listing_list(client, token=None)
...
async def test_authenticated_without_permission_returns_403(kernel: Kernel, client: httpx.AsyncClient) -> None:
    ...
    response = await _invoke_listing_list(client, token)
...
@pytest.mark.asyncio
async def test_authenticated_with_permission_returns_200_and_empty_catalog(
    ...
    await _grant_role_permission(storage.data, _TEST_ROLE, "marketplace:read")
    token = await _login(client, tenant_id, "principal-1")
