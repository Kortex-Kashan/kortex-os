"""M6 real-HTTP regression coverage for the Workflow Registry.
...
M6 requires:
    ...
    valid session token, "workflow:read"        -> 200, payload.result == []

See `test_connector_api_http.py`'s module docstring for why `httpx.ASGITransport`
...
_TEST_ROLE = "workflow-http-test-role"
_CREDENTIAL = "workflow-http-test-credential"


@pytest_asyncio.fixture
async def kernel(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> AsyncIterator[Kernel]:
    monkeypatch.setenv("KORTEX_STORAGE_DIR", str(tmp_path / "workflow_http_storage"))
...
def _tenant() -> str:
    return f"tenant-workflow-http-{uuid.uuid4().hex[:8]}"
...
async def _invoke_definition_list(client: httpx.AsyncClient, token: str | None) -> httpx.Response:
...
async def test_no_authorization_header_returns_401(client: httpx.AsyncClient) -> None:
    response = await _invoke_definition_list(client, token=None)
...
async def test_authenticated_without_permission_returns_403(kernel: Kernel, client: httpx.AsyncClient) -> None:
    ...
    response = await _invoke_definition_list(client, token)
...
async def test_authenticated_with_permission_returns_200_and_empty_registry(
    ...
    await _grant_role_permission(storage.data, _TEST_ROLE, "workflow:read")
    token = await _login(client, tenant_id, "principal-1")
