...
3. **Nested Dictionaries**: Flattended into tagged series if key matches a permitted label (e.g. `{"per_driver_executions": {"postgres": 5}}` becomes series `connector.executions{driver="postgres"} = 5`).
