...
**New**: `backend/tests/unit/test_base_module.py` (7 tests — construction, initialize→ACTIVE, start/stop state guards, health_check, `ensure_state` error message); `backend/tests/unit/test_finance_invoice_validation.py` (7 tests — valid request, blank/whitespace customer name, zero/negative amount, malformed currency); `backend/tests/integration/test_finance_invoice_capability.py` (5 tests — real dispatch + persistence, tenant-authority adversarial, no-token/no-permission denial, invalid-input rejection).
...
- **Discovered, not used**: `RegistryEngine` already has an unused `register_module`/`RegistryCategory.MODULE` pair (`registry/engine.py`), a more conceptually precise registration hook than reusing the `ENGINE` category via `kernel.register_engine()`. Not wired up in this slice — the master prompt explicitly directed following the existing *engine*-registration pattern literally, and adding a `Kernel.register_module()` convenience wrapper was not requested. Flagged as a real, evidenced follow-up for a future pass, not a defect.
