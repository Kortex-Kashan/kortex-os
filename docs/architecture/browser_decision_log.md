...
- **OD-B5**: Root-cause the `STATUS_ENTRYPOINT_NOT_FOUND` crash that blocks `tauri`'s `test` feature + `unstable` feature from coexisting in an automated test binary on this development machine (D12), and restore automated `Window`/`Webview` lifecycle test coverage. Not resolved in Browser-B1; recommended before Browser-B2 or B3 add meaningfully more runtime logic that would benefit from the same kind of test.
