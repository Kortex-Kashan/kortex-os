...
**Last updated**: Integration Hub M2 Acceptance & Defect Resolution Reconciliation pass, against HEAD `283cf87fab6ffe2624bbd8fef5d0bc7826c4620a`. This pass formally records acceptance of Integration Hub M2 (GitHub OAuth Connector) by Chief Architect KASHAN (§5.13), and records that the DEFECT-002 workflow-durability restart-recovery race condition (§5.12) is **RESOLVED** by commit `6cc223be7ebcfe17269408791ae532b6dd5d039e` (root cause was in `DatabaseEngineManager`'s lack of application-level SQLite writer serialization, not the Workflow Engine) and that DEFECT-003 (stale Update Engine manifest test fixture timestamps, discovered during M2) is **RESOLVED** by commit `283cf87fab6ffe2624bbd8fef5d0bc7826c4620a`. Also records the MCP mypy CI hygiene fix, commit `bdb94614a339dd852386169d1dbf133e56185d03`, as repository-derived supporting work. Immutable RC tag `v1.0.0-rc.1` (`9d68ec7c262c4b0085bf7adf056855082e4759fe`) preserved untouched; this pass makes no release-identity, signing, or version-manifest change. Prior passes: Integration Hub M1 Acceptance & CI Defect Reconciliation pass (`79040e9270cdf87b0bc616b6f84f2cd8679d0f72`); Final RC Ambiguity Resolution & Baseline Freeze pass (`86469c97820713ff9774973319d28331a51b55b7`); Final Production Reconciliation (`98d94b4c4da9cbec0b3c50af6894af88f8796aae`). Consolidated release view: `docs/release/RELEASE_CANDIDATE_READINESS.md`. Defect tracking: `docs/release/rc-testing/KNOWN_FINDINGS.md`.
...
2. **Deployment topology** — **RESOLVED**. Both bullets are real and accepted: Docker production builds (§5.7) and Desktop Installers (§5.8), confirming the roadmap's two-topology reading. The narrower question §5.7 flagged — **OD-2, Docker secret-persistence parity with the desktop sidecar's OS-keychain mechanism** — is now also **RESOLVED: DEFERRED / POST-RC**, with this definitive architectural statement:

   > **Docker production v1 uses the accepted operator-supplied-secret model. Native OS credential-store parity with the Windows desktop keyring is not an RC requirement and remains future release-engineering work.**

   Basis: the accepted Docker architecture (§5.7) already ships a fail-closed operator-supplied-secret posture (`KORTEX_MASTER_KEY`/`KORTEX_AUTH_SIGNING_PRIVATE_KEY` via container secrets/env, with the entrypoint refusing to start if absent), which is the standard and appropriate mechanism for a containerized deployment — a container has no user-session credential store to bind to, which is precisely why the desktop keyring mechanism does not transfer. No repository evidence indicates Docker requires desktop-keychain parity for RC. This is now a single, definitive status; it is no longer "open". No Docker change was made, no secret-management subsystem was added, and the accepted Docker architecture is untouched. Distinct from §5.11's desktop-side defect fix, which concerned whether the desktop's own keychain mechanism actually functioned (it did not; it now does).
...
| Integration Hub M1 (MCP Foundation + Capability Projection Bridge) | **M1 — ACCEPTED / COMPLETE** (PRE-EXISTING CI DEFECT RESOLVED) | Baseline 63460cb | §5.12 | Formally accepted by Chief Architect; pre-existing workflow durability defect (DEFECT-002) resolved by `6cc223b` (§5.12) |
| Integration Hub M2 (GitHub OAuth Connector) | **M2 — ACCEPTED / COMPLETE** | Integration Hub M1 (`da93c64`, `79040e9`) | §5.13 | Formally accepted by Chief Architect; both defects discovered alongside this milestone (DEFECT-002, carried over from M1; DEFECT-003, new) resolved (§5.13) |
...
**Definitive statuses for the items this acceptance does not itself deliver** (each has exactly one status — none is left ambiguous):
- **OD-DI-2 (code signing)** — **TECHNICAL RC: NOT BLOCKED. PUBLIC DISTRIBUTION: SIGNING REQUIRED.** Both MSI and NSIS are **unsigned**. No certificate, signing identity, or CI signing credential exists anywhere in the repository — confirmed by direct inspection of `tauri.conf.json`/`tauri.windows.conf.json` (`certificateThumbprint`/`signingIdentity` absent) and `.github/workflows/desktop-ci.yml` (no signing step). Unsigned artifacts build, install, launch, and run correctly — proven repeatedly in CI on fresh Windows runners — so signing does not block technical RC freeze. It does gate public distribution: Windows SmartScreen/Defender will warn on an unsigned installer, and users have no cryptographic proof of publisher identity or artifact integrity. See `docs/release/RELEASE_CANDIDATE_READINESS.md` §14 for the full signing requirements breakdown (what it protects, which artifacts need it, what infrastructure and credentials are missing, and where those credentials must live — never in this repository).
- **OD-DI-3 (macOS/Linux desktop distribution)** — **DEFERRED / FUTURE PLATFORM EXPANSION.** Not built, not faked, no placeholder implementation, no non-executable CI job added. Windows x64 is the only desktop target in the RC distribution scope. The roadmap's "Desktop installers (Tauri .msi / .exe / .dmg)" bullet is checked off against the Windows scope only, with the `.dmg` omission stated explicitly in the roadmap itself rather than glossed over.
...
### 5.10 Fresh-Machine Production Validation — SPLIT: two topologies PASS, one DEFERRED / POST-RC

**Definitive classification** (this item was previously a single ambiguous `PENDING`; it is now split by topology, because the three cases have genuinely different evidence):

- **A. Windows Desktop fresh-machine validation — PASS.** Covered by real evidence, not deferred: the `windows-installer` CI job installs the NSIS package onto a fresh `windows-latest` runner (no prior KORTEX state of any kind) and drives install → launch → `/health` on every push; locally, the full 16-stage lifecycle was run twice consecutively from a genuinely cleared app-data state. The `f55b6bb` defect (missing `storage_data/` on a fresh install) was found *precisely because* fresh-machine conditions are actually exercised — this is not a theoretical gap.
- **B. Docker fresh-machine validation — PASS.** Covered by the Backend CI `docker` job: a clean image is built and a container started from scratch on a fresh runner, with Alembic migrations running against an empty volume, `/health` verified, non-root runtime confirmed, and image layers scanned for secrets/VCS metadata — on every push.
- **C. Bare/server (non-Docker, non-desktop) fresh-machine validation — DEFERRED / POST-RC DEPLOYMENT HARDENING.** This is the only remaining case, and it is **not part of the RC distribution topology**: the supported RC targets are the Windows desktop installer and the Docker/headless production container, both of which are validated above. The concerns recorded below (ephemeral-key fallback, cwd-relative `_DEFAULT_STORAGE_DIR`) are real but apply only to a bare, unmanaged server deployment that KORTEX does not ship or support in this RC. **Technical RC is not blocked by an environment outside the supported RC distribution topology.** Should a bare/server topology be authorized later, this item becomes live again.

**Original analysis (retained, still accurate for case C):**
...
### 5.12 Integration Hub M1 — MCP Foundation + Capability Projection Bridge — ACCEPTED / COMPLETE (PRE-EXISTING CI DEFECT RESOLVED)
...
- **Backend full suite (at M1 acceptance time)**: 1 pre-existing failure (3,989 passed, 2 skipped, 1 failed out of 3,992 tests)
- **Overall repository CI (at M1 acceptance time)**: NOT GREEN due to pre-existing workflow durability defect (`DEFECT-002`). **Now RESOLVED** — see below and §5.13.

**Pre-Existing Baseline Defect Classification — RESOLVED**:
1. **Defect**: An optimistic-lock concurrency race condition existed in the workflow durability restart recovery mechanism:
...
2. **Baseline Reproduction**: Reproduced on the M1 parent baseline commit `63460cb792222ab2796615afbb9498dd6f389ed`.
3. **Current Commit Reproduction (at the time)**: Also reproduced on M1 commit `79040e9270cdf87b0bc616b6f84f2cd8679d0f72`.
4. **Classification**: **PRE-EXISTING BASELINE DEFECT**. Definitively **NOT an M1 regression**.
5. **Scope Ownership (as originally classified)**: Attributed to Workflow Engine (`kortex.engines.workflow`) durability/persistence. **Root-cause correction upon resolution**: the actual defect was one layer lower, in `DatabaseEngineManager` (`backend/src/kortex/core/db.py`) — SQLite allows only one writer transaction at a time, but the application layer never enforced that, so two concurrent `get_session()` callers could interleave transactions against the shared connection. The Workflow Engine's restart-recovery path was the reachable symptom, not the fault's origin.
6. **Resolution**: **RESOLVED** by commit `6cc223be7ebcfe17269408791ae532b6dd5d039e` (`fix(storage): serialize concurrent SQLite sessions`) — serializes the SQLite session lifetime behind a per-event-loop-bound `asyncio.Lock`, leaving PostgreSQL's genuine per-connection concurrency untouched. Regression coverage added: a storage-layer concurrent-write stress test, a Workflow Engine restart-recovery stress test, and two lock re-entrancy-boundary tests. A pre-existing test-design race in `test_workflow_definition_version_safety.py`, newly exposed once this defect stopped masking it, was fixed in the same commit.
7. **Integrity Rule (honored)**: was never silently treated as an M1 failure, and M1 acceptance was not withheld pending its resolution.
8. **CI State**: No longer a known CI failure as of `6cc223b`. Full detail: `docs/release/rc-testing/KNOWN_FINDINGS.md` DEFECT-002.

### 5.13 Integration Hub M2 — GitHub OAuth Connector — ACCEPTED / COMPLETE

**Status**: **M2 — ACCEPTED / COMPLETE**

**Formal acceptance record**:
- **Chief Architect decision**: Integration Hub M2 is formally **ACCEPTED** by Chief Architect KASHAN.
- **Accepted implementation commit**: `721d30a` (`feat(integration): add GitHub OAuth connector`).
- **Follow-on commits within the same milestone's CI-green-up sequence**: `e3a1dc6` (`fix(integration): satisfy backend CI lint/type checks, fix MCP resolver arity`), `6cc223b` (`fix(storage): serialize concurrent SQLite sessions` — DEFECT-002 resolution, §5.12), `bdb9461` (`build(backend): fix pre-existing mypy failure for mcp imports`), `283cf87` (`test(update): fix DEFECT-003 stale Update Engine manifest test fixture timestamps`).
- **Parent baseline**: Integration Hub M1 (`da93c64`, `79040e9`, §5.12).

**Components Delivered & Accepted** (per `docs/architecture/integration_hub_m2_github_oauth_connector_implementation_report.md`): `IntegrationOAuthManager` (SecurityEngine-owned OAuth begin/complete/status/disconnect-credential, GitHub refresh with mandatory rotation, per-`(tenant_id, profile_id)` synchronization); `OAuthIntegrationCredentialRecord`/`OAuthStateNonceRecord` models with atomic single-use OAuth `state` consumption; `GitHubIntegrationOAuthProvider` (Authorization Code flow, `offline_access`); GitHub curated action catalog (`user_get`, `repo_get`, `issues_list`, `issue_create`) with dynamic per-profile capability registration (`kortex.connector.<profile_id>.<action>`), following M1's dynamic-capability pattern rather than a boot-time-fixed one; a single backend-authoritative `kortex.connector.integration.disconnect` capability; a third `profile_id` parameter added to `secret_resolver` for profile-bound credential resolution; desktop "Connect with GitHub"/"Disconnect" UI. Design went through three rounds of Chief-Architect architecture correction before implementation (capability identity, disconnect atomicity, credential-profile binding, OAuth session binding, GitHub token-lifecycle handling) — see the implementation report §3 for the full record.

**Verification & CI Status** (at M2 implementation time):
...
- **Backend full suite**: 4023 passed, 4 failed, 2 skipped.
- **Overall repository CI (at the time)**: NOT GREEN due to two pre-existing baseline defects, neither an M2 regression (both verified via `git stash` against unmodified `main` HEAD `f5d57cb`): DEFECT-002 (carried over from M1) and DEFECT-003 (new, Update Engine test-fixture staleness).

**DEFECT-003 — Hardcoded, now-expired Update Engine manifest fixture — RESOLVED**:
1. **Defect**: `test_update_integration.py`'s `create_signed_package` helper constructed `UpdateManifest` fixtures with a hardcoded `expires_at: "2026-09-12T00:00:00Z"`. Once real wall-clock time passed that timestamp, `UpdateManifest.parse_dict` (correctly) rejected the manifest as expired, failing 3 tests (`test_end_to_end_successful_update_lifecycle`, `test_recovery_delegation_on_post_mutation_failure`, `test_checkpoint_failure_aborts_before_any_destructive_mutation`).
2. **Classification**: PRE-EXISTING BASELINE DEFECT (test-fixture data staleness, not an Update Engine logic defect) — reproduced on unmodified `main` HEAD `f5d57cb` via `git stash` isolation, confirmed unrelated to Integration Hub M2's scope (`kortex.engines.security.*`, `kortex.engines.connector.*`, desktop Connectors UI touch no file under `kortex.engines.update` or `test_update_integration.py`).
3. **Resolution**: **RESOLVED** by commit `283cf87fab6ffe2624bbd8fef5d0bc7826c4620a` (`test(update): fix DEFECT-003 stale Update Engine manifest test fixture timestamps`) — `create_signed_package` now computes timezone-aware UTC `created_at`/`expires_at` timestamps relative to execution time instead of a fixed calendar date.
4. **CI State**: No longer a known CI failure as of `283cf87`. Full detail: `docs/release/rc-testing/KNOWN_FINDINGS.md` DEFECT-003.

**MCP mypy CI hygiene fix** — **REPOSITORY-DERIVED REQUIREMENT**, not itself M2 engineering: commit `bdb94614a339dd852386169d1dbf133e56185d03` (`build(backend): fix pre-existing mypy failure for mcp imports`) added a targeted `backend/pyproject.toml` mypy override (`ignore_missing_imports` for `mcp.*`) because the `mcp` library (Integration Hub M1) ships no type stubs or `py.typed` marker, which was aborting `mypy src` before `pytest` could run in CI. No production logic changed.

**Current reconciled state (this pass)**: with DEFECT-002 and DEFECT-003 both resolved and the mypy CI gate fixed, Backend CI is green at HEAD `283cf87` (full pytest suite, lint, and type-check). Docker build and smoke test also pass at this HEAD. See §11/§12 of `docs/release/RELEASE_CANDIDATE_READINESS.md` for the consolidated evidence table; exact current full-suite pass/fail counts and a GitHub Actions run identifier for `283cf87` were not independently re-verified by this reconciliation pass (no `gh` CLI access in this session) and are cited here only as commit-level, not run-level, evidence.
...
## 8. Production Gate — two distinct states, deliberately not collapsed

This gate distinguishes **technical RC readiness** (is the engineering complete enough to freeze a Release Candidate build?) from **public production release readiness** (may those artifacts be distributed publicly?). Conflating the two is what previously made "production-ready" ambiguous.

### 8.1 Technical RC Gate — **SATISFIED**

- **EXISTING ROADMAP REQUIREMENT**: all seven Phase 7 checklist items implemented and now checked off in `.kortex/roadmap.md`. **Roadmap synchronization: RESOLVED.** The prior contradiction (roadmap reading `Status: Planned`/all-unchecked while this document recorded `DONE`) no longer exists. It was resolved under the roadmap's *own* stated convention — that file explicitly records, on its own previously-pending Phase 4/5 items, that an item is "not checked off `[x]` until Chief Architect review and explicit acceptance." That acceptance was given, so checking the boxes follows the roadmap's rule rather than overriding it. This document's §0 prohibition on *silently* rewriting the roadmap is honored: the change is neither silent nor unilateral — the roadmap carries its own explicit synchronization note recording the governance basis.
- **REPOSITORY-DERIVED REQUIREMENT**: a real, tested schema-upgrade path exists (§5.1 — satisfied).
- **REPOSITORY-DERIVED REQUIREMENT**: the ephemeral-key-fallback risk is resolved or explicitly scoped out for every topology in the RC distribution scope — **satisfied**. Desktop sidecar: resolved (§5.11, real OS keyring). Docker: resolved by the accepted fail-closed operator-supplied-secret model (§5.7, OD-2 now definitively DEFERRED / POST-RC per §3). Bare/server: explicitly outside the RC distribution topology (§5.10 case C).
- **Fresh-machine validation**: satisfied for both supported topologies (§5.10 cases A and B).

**Conclusion: TECHNICAL RC READY.** No core-engine implementation remains outstanding for RC.

### 8.2 Public Production Release Gate — **PENDING RELEASE IDENTITY / SIGNING POLICY**

- **Code signing** — **PUBLIC RELEASE BLOCKER** (not a technical-RC blocker). MSI and NSIS are unsigned; no signing infrastructure exists. See §5.8's OD-DI-2 status and `docs/release/RELEASE_CANDIDATE_READINESS.md` §14.
- **Release identity (version/tag)** — **OWNER DECISION REQUIRED**. No RC tag convention exists; package manifests sit at `0.1.0` while `CHANGELOG.md`'s last released version is `0.2.0` and `ARCHITECTURE_VERSION_1.0.md` records a frozen, ratified architecture version of `1.0.0`. A single recommended RC identifier and the exact files a bump would touch are documented in the RC readiness document; neither the bump nor the tag was performed, since neither is authorized.

**Conclusion: PUBLIC RELEASE PENDING SIGNING / RELEASE IDENTITY.** These are release-policy and distribution concerns, not core product engineering.
